...
(() => {
  ...
  function renderVariants(){
    ...
    $('ig-photo-usage').textContent='Dieses Quellenfoto ist im Kalender: '+usage+'. Vormerkungen bleiben bestehen; der Standardexport lässt sie aus.';
  ...
  function renderPlan(){
    ...
    $('ig-plan-list').innerHTML=selected.map(item=>{
      ...
      return `<article class="ig-plan-row" data-plan="${escape(item.id)}"><img src="${escape(asset(item.asset))}" alt="${escape(item.title)}" loading="lazy"><div class="ig-plan-info"><h3>${escape(item.place)}</h3><p>${escape(label(item))}</p>${item.palette?'<p>'+escape(item.palette)+'</p>':''}${usage?'<p class="ig-conflict">Kalender: '+escape(usage)+'<br>Im Standardexport ausgelassen</p>':''}<button type="button" class="ig-remove" data-remove="${escape(item.id)}" ${blocked?'disabled':''}>Vormerkung entfernen</button></div><label class="ig-field ig-note-field" for="note-${escape(item.id)}">Idee oder Notiz<textarea id="note-${escape(item.id)}" data-note="${escape(item.id)}" rows="3" ${blocked?'disabled':''}>${escape(itemValue.note)}</textarea></label><label class="ig-field ig-month-field" for="month-${escape(item.id)}">Planmonat (optional)<select id="month-${escape(item.id)}" data-month="${escape(item.id)}" ${blocked?'disabled':''}><option value="">Noch offen</option>${months.map((month,index)=>'<option value="'+(index+1)+'" '+(itemValue.month===index+1?'selected':'')+'>'+month+'</option>').join('')}</select></label></article>`;
    }).join('');
  }
